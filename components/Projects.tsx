...
const projects: ProjectCardProps[] = [
  {
    ...
    description:
      "An ethical ML system for tracking media representation using self-identification-only data. Built on the principle that identity classification, everywhere including in ML, should come from how people describe themselves and never be derived from appearance alone.",
    highlights: [
      {
        label: "Safety design",
    ...
    highlights: [
      {
        label: "Intake",
        detail:
          "iOS Shortcut captures handwritten or typed notes; OCR output is parsed and normalized by Claude before task creation",
      },
      {
        label: "Task layer",
        detail:
          "Notion as the human-facing task UI, with a Supabase backend as the source of truth for scheduling logic",
      },
      {
        label: "AI scheduling",
        detail:
          "Claude reads calendar availability and task metadata to propose time blocks — no manual calendar tetris",
      },
      {
        label: "Reliability",
        detail:
          "APScheduler runs nightly sync and Notion polling on Fly.io; auto-deploys from GitHub Actions",
      },
    ],
    tags: [
      "Python",
  ...
  },
  {
    ...
    description:
      "Tether re-centers how you connect with people. iOS-first, with a Flutter mobile client and a FastAPI backend.",
    highlights: [
      {
        label: "Matching",
      ...
      },
      {
        label: "Architecture",
        detail:
          "Feature-first Flutter frontend; FastAPI service layer with strict repository pattern — no ORM calls outside repositories",
      },
      {
        label: "Realtime",
        detail:
          "In-app messaging via Supabase Realtime; row-level security enforced at the database layer",
      },
    ],
    tags: [
      "Flutter",
